...
function useLocalStorage<T>(key: string, initialValue: T): [T, (value: T) => void] {
  ...
  useEffect(() => {
    try {
      ...
      if (item !== null) {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- sync with localStorage on client mount
  ...
  if (!mounted) {
    return (
      <div className="flex min-h-screen">
        <main className="flex-1 p-4 pt-16 md:pt-4 sm:p-6 md:p-8 md:ml-20">
          {children}
        </main>
      </div>
    );
