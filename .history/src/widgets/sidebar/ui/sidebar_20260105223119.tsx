...
import { LayoutDashboard, FileText, History, ChevronsLeft } from 'lucide-react';
...
const navItems: NavItem[] = [
  { href: '/', label: 'Дашборд', icon: <LayoutDashboard className="w-5 h-5" /> },
  { href: '/reports', label: 'Генерация', icon: <FileText className="w-5 h-5" /> },
  { href: '/history', label: 'История', icon: <History className="w-5 h-5" /> },
];
...
function ToggleButton({ collapsed, onToggle }: ToggleButtonProps) {
  return (
    <button
      onClick={onToggle}
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center mb-6 ${collapsed ? 'justify-center' : 'justify-between'}`}>
        ...
        {collapsed && (
          <span className="text-xl font-bold text-gray-900 dark:text-white">JC</span>
        )}
        {/* Mobile close button */}
        {onClose && (
          <button
      ...
      <div className="pt-4 border-t border-gray-200 dark:border-gray-700 space-y-3">
        {!collapsed ? (
          <>
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-500 dark:text-gray-400">Свернуть</span>
